...
- For local browser testing, check for an existing suitable app server before starting `pnpm dev:docker`; reuse it when it already points at the intended config, choose another port when needed, and stop only the process you started. `pnpm dev:docker` regenerates config with local Docker overrides before starting Next.js. Sign in on `/login` with the seeded Admin or Member email/password panel, and restore generated config afterward with plain `pnpm generate:config` before committing.
