...
- Supabase integrity verifier coverage: extend the local checker beyond current item-reference foreign keys and invite-code uniqueness as owner, visibility, versioning, moderation, and image tables land. Impact: catches data-integrity regressions before live migrations. Uncertainty/research: confirm live-project drift and migration order with Supabase metadata before applying production changes.
