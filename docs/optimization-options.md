...
- Guided operator bootstrap: add an interactive `pnpm setup:operator` or richer `pnpm create:deployment` flow that asks for domain, GitHub owner/repo, Supabase URL/key, Auth redirect path, optional `deploy/<slug>` branch, and whether to generate content placeholders. Impact: gets fork operators from fork to working app faster. Uncertainty/research: confirm how much can be automated safely without writing secrets or hiding required Supabase dashboard steps.
