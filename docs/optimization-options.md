...
- Test strategy: extend focused tests beyond config generation and app-config helpers to Supabase RPC contracts, upload validation, and critical UI state. Impact: safer refactors. Side effect: respect the user's preference for agentic browser skills before adding browser-test packages.
