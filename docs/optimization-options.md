...
- Generated backend config: media MIME and size limits currently exist in both `config/bringa.config.jsonc` and Supabase Storage SQL. Impact: reduces deployment drift if future tooling generates or checks backend settings from config. Uncertainty/research: wait until Supabase MCP confirms live bucket metadata and migration workflow.
