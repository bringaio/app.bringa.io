...
- Test strategy: extend focused tests beyond config generation, app-config helpers, media policy helpers, and Supabase contract checker behavior to critical UI state. Impact: safer refactors. Side effect: respect the user's preference for agentic browser skills before adding browser-test packages.
